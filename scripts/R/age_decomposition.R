...
chars               <- c('P0', 'P20', 'P40', 'P60', 'P80')
scenario            <- 'SSP1' 
outputs_dir         <- file.path('outputs', 'decomposition')
chars_file          <- file.path(outputs_dir, str_c('age_', scenario, '.gpkg'))
char_output_dir     <- file.path(outputs_dir, 'age', scenario) 
...
read_dem_sf <- function(dem_file, year){
  ...
  chars_sf <- dem_file %>%
    ...
    mutate(row_sums = rowSums(.[, 1:5, drop=TRUE])) %>%
...
population_bor_df[, age_group := age_group]
