...
derive_int_in_migration <- function(inputs_path, cur_year, pop_df, cur_scenario){
  ...
  for (region in regions){
    ...
    cur_int_mig_rates_df <- file.path(inputs_path, region, 'internal_in_migration.csv') %>%
      fread() %>%
    ...
    cur_int_mig_rates_df <- cur_int_mig_rates_df[Origin != region, ]
    ...
    cur_int_mig_values_df <- cur_origin_pop_df[cur_int_mig_rates_df, 
                                             .(Origin, Destination = region, Race, Sex,
                                               Age, Population, Internal.migration.rate,
                                               int_in_mig = Population * Internal.migration.rate),
                                             on = c('Region' = 'Origin', 'Race', 
                                                    'Sex', 'Age')]
    
    
    # Sum the total in migration numbers across all contributing regions
    cur_tot_int_mig_values_df <- cur_int_mig_values_df[, .(int_in_mig = sum(int_in_mig)),
